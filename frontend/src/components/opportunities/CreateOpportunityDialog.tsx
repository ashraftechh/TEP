...
import { createOpportunity, clearCreateStatus } from '@/store/slices/opportunitySlice';
...
export const CreateOpportunityDialog: React.FC<CreateOpportunityDialogProps> = ({
  ...
  const { isCreating, createErrorCode, validationErrors } = useAppSelector(
  ...
  const {
    ...
    defaultValues: {
      ...
      department_ar: '',
      department_en: '',
      opportunity_type_id: opportunityTypes[0]?.id ? String(opportunityTypes[0].id) : '',
      training_cycle_id: '',
      location: '',
      duration: '',
      capacity: 1,
  ...
  // Set default opportunity type when types load async (after initial render)
  useEffect(() => {
    if (opportunityTypes.length > 0 && !getValues('opportunity_type_id')) {
      setValue('opportunity_type_id', String(opportunityTypes[0].id));
    }
  }, [opportunityTypes, setValue, getValues]);
  ...
  const onSubmit = async (values: OpportunityFormValues) => {
    ...
    const result = await dispatch(createOpportunity(payload));

    if (createOpportunity.fulfilled.match(result)) {
      toast.success(t('opportunities:form.successMessage'));
      handleClose(false);
      onSuccess?.();
    } else if (createOpportunity.rejected.match(result)) {
      const errMsg =
        result.payload?.message ||
        (isRTL
          ? 'فشل إنشاء الفرصة التدريبية. يرجى التحقق من صحة البيانات المدخلة.'
          : 'Failed to create training opportunity. Please check the entered data.');
      toast.error(errMsg);
    }
  };
        ...
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-foreground-muted">{t('opportunities:subtitle')}</p>
            </div>
          </div>
        </DialogHeader>

        {/* Company Not Approved Warning Alert */}
        {createErrorCode === 'company_not_approved' && (
          ...
          <DialogFooter>
            ...
            >
              {isCreating ? (
                ...
                t('opportunities:form.saveDraft')
              )}
            </Button>
          </DialogFooter>
        </form>
